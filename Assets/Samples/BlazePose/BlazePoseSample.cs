...
public sealed class BlazePoseSample : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        score = 0;
        detected = false;
        RawImage img = cam.GetComponent<RawImage>();
        img.enabled = true;
        RawImage uiimg = ui.GetComponent<RawImage>();
        uiimg.enabled = true;
    }
    ...
    private void Update()
    {
        if (landmarkResult != null && landmarkResult.score > 0.2f && !detected)
        {
            detected = true;
            RawImage img = cam.GetComponent<RawImage>();
            img.enabled = false;
            ...
            if (poseResult != null && poseResult.score > 0f)
        {
            ...
            bend = poseResult.keypoints[1][0] - poseResult.keypoints[0][0];
        }
        ...
        //scoretext.text = landmarkResult.viewportLandmarks[1].ToString();
